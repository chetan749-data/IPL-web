...
{/* <div>
                    <h3 class="name">Name:MS dhoni</h3>
                    <img class="photo" src="https://static.iplt20.com/players/210/1.png" alt="">
                    <h3 class="team">team: CSK</h3>
                    <h3 class="price">price: 12.5cr</h3>
                    <h3 class="sttaus">Status:Playing</h3>
                    <h3 class="role">Role: Batsman</h3>
                </div> */}
                // var div=document.createElement('div')
                // div.classList('design')
                // var heading=document.createElement('h3')
                // heading.classList('name')
                // heading.innerHTML="xyz"
                // var image=document.createElement('img')
                // image.classList('photo')
                // image.src="https://static.iplt20.com/players/210/1.png"
                var main=document.getElementById('main')
                for(let i=0;i<player.length;i++)
                {
                    if(player[i].from=='CSK')
                {

                    
                    var div=document.createElement('article')
                div.classList.add('csk-players')
                var heading=document.createElement('h4')
                heading.classList.add('name')
                heading.innerHTML=player[i].playerName
                var image=document.createElement('img')
                image.classList.add('photo')
                image.src=player[i].image
                var team=document.createElement('h4')
                team.classList.add('team')
                team.innerHTML=player[i].from
                var price=document.createElement('h4')
                price.classList.add('price')
                price.innerHTML=player[i].price
                var description=document.createElement('h4')
                description.classList.add('role')
                description.innerHTML=player[i].description
               
                // var status=document.createElement('h4')
                // status.classList.add('status')
                // status.innerHTML=player[i].isPlaying
